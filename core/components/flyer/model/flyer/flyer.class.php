...
class flyer
{
    ...
    function __construct(modX &$modx, array $config = array())
    {
        ...
        $corePath = $this->getOption('core_path', $config,
            $this->modx->getOption('core_path', null, MODX_CORE_PATH) . 'components/flyer/');
        $assetsUrl = $this->getOption('assets_url', $config,
            $this->modx->getOption('assets_url', null, MODX_ASSETS_URL) . 'components/flyer/');
        ...
        $this->modx->addPackage('flyer', $this->getOption('modelPath'));
        $this->modx->lexicon->load('flyer:default');
        $this->namespace = $this->getOption('namespace', $config, 'flyer');
    }
    ...
    public function getOption($key, $config = array(), $default = null, $skipEmpty = false)
    {
        ...
        if (!empty($key) AND is_string($key)) {
            ...
            } elseif (array_key_exists("flyer_{$key}", $this->modx->config)) {
                $option = $this->modx->getOption("flyer_{$key}");
            }
        }
    ...
    public function initialize($ctx = 'web', array $config = array())
    {
        ...
        if ((!defined('MODX_API_MODE') OR !MODX_API_MODE)) {
            ...
            if ($useEditor AND $whichEditor == 'flyer') {
                $initialize = true;
            }
        }
    ...
    public function loadControllerJsCss(
        ...
        if (!empty($set['config'])) {
            ...
            <script type='text/javascript'>
                Ext.ns('flyer');
                flyer.config={$this->modx->toJSON($config)};
                flyer.editorConfig = {$this->modx->toJSON($editorConfig)};
                Ext.onReady(function(){
                    Ext.getBody().addClass('flyer-body');
                    flyer.loadEditorForFields();
                });
            </script>";
        }
        ...
        if (!empty($set['tools'])) {
            ...
            $controller->addJavascript($this->config['jsUrl'] . 'mgr/flyer.js');
        }
        ...
        $controller->addLexiconTopic('flyer:default');

        $this->modx->invokeEvent('flyerOnLoadControllerJsCss', array('controller' => &$controller));
    ...
    public function addConfigSetting(array $row = array(), $addVariable = true)
    {
        $row = array_merge(array(
            'key'   => 'flyer_undefined_undefined',
            'area'  => 'flyer_undefined',
            'type'  => 'undefined',
            'value' => ''
        ), $row);
